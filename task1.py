...
def convolve2d(img, kernel):
    ...
    for i in range(1,len(img)-1):
        for j in range(1,len(img[0])-1):
            img_inter=utils.elementwise_mul(utils.crop(img,i-1,i+2,j-1,j+2),kernel)
            #print(img_inter)
            sum=0
            for k in [0,1,2]:
                for l in [0,1,2]:
            ...
            #print(sum)
            img_conv[i][j]=sum
    img_conv=utils.crop(img_conv,1,len(img_conv)-1,1,len(img_conv[0])-1)
...
def edge_magnitude(edge_x, edge_y):
    ...
    """
    edge_mag=np.sqrt(np.add((edge_x**2),(edge_y)**2))
