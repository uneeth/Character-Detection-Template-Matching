...
def flip2d(img, axis=None):
    ...
    """
    if(axis==0):
        flipped_img = filp_x(img)
    elif(axis==1):
        flipped_img = flip_y(img)
    else:
        flip_img_x=flip_x(img)
        flipped_img=copy.deepcopy(flip_img_x)
        center = int(len(flip_img_x[0]) / 2)
        for i, row in enumerate(flip_img_x):
            for j in range(center):
                flipped_img[i][j] = flip_img_x[i][(len(flip_img_x[0]) - 1) - j]
                flipped_img[i][(len(flip_img_x[0]) - 1) - j] = flip_img_x[i][j]
